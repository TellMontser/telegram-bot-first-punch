...
export async function addPaymentMethod(userId, paymentMethodId, type = 'card', cardMask = null, autoPaymentsEnabled = false, autoPaymentInterval = 300) {
  try {
    ...
    const nextPaymentDate = autoPaymentsEnabled 
      ? new Date(Date.now() + autoPaymentInterval * 1000).toISOString()
      : null;
    ...
    const { data, error } = await supabase
      ...
      .insert({
        ...
        auto_payment_interval: autoPaymentInterval,
...
export async function updatePaymentMethodNextDate(paymentMethodId, nextPaymentDate) {
  try {
    console.log(`🔄 Обновление даты следующего платежа: ${paymentMethodId}`);
    ...
    if (error) throw error;
    console.log(`✅ Дата следующего платежа обновлена: ${paymentMethodId}`);
    return true;
  } catch (error) {
    console.error('❌ Ошибка при обновлении даты платежа:', error);
    return false;
  }
}

export async function disablePaymentMethod(paymentMethodId) {
  try {
    console.log(`🚫 Отключение способа оплаты: ${paymentMethodId}`);
    ...
    if (error) throw error;
    console.log(`✅ Способ оплаты отключен: ${paymentMethodId}`);
    return true;
  } catch (error) {
    console.error('❌ Ошибка при отключении способа оплаты:', error);
    return false;
  }
}

export async function getPaymentMethodsForAutoPayment() {
  try {
    ...
    console.error('❌ Ошибка при получении способов оплаты для автоплатежа:', error);
    return [];
  }
}
...
export async function addSubscription(userId, paymentId, amount, duration = 30, paymentSystem = 'yukassa', paymentMethodId = null, subscriptionType = 'manual') {
  try {
    ...
    // Получаем количество месяцев подряд
    ...
    const { data, error } = await supabase
      ...
      .insert({
        ...
        consecutive_months: consecutiveMonths + 1,
        created_at: new Date().toISOString(),
        updated_at: new Date().toISOString()
      })
      .select()
    ...
    console.log(`✅ Подписка создана для пользователя: ${userId} через ${paymentSystem} (${consecutiveMonths + 1} месяцев подряд)`);
    return data;
  } catch (error) {
...
// Функция для подсчета месяцев подписки подряд
export async function getConsecutiveMonthsCount(userId) {
  try {
    ...
    for (const subscription of subscriptions) {
      ...
      // Если это первая подписка или есть разрыв больше 35 дней
      if (consecutiveMonths === 0) {
        consecutiveMonths = 1;
        lastEndDate = startDate;
      } else {
        const daysDiff = Math.abs(lastEndDate.getTime() - endDate.getTime()) / (1000 * 60 * 60 * 24);
        if (daysDiff <= 35) { // Допускаем небольшую задержку в оплате
          consecutiveMonths++;
          lastEndDate = startDate;
        } else {
          break; // Прерываем подсчет при разрыве
        }
      }
    }
    ...
    console.error('❌ Ошибка при подсчете месяцев подряд:', error);
    return 0;
  }
}
...
export async function getStats() {
  try {
    ...
    const yukassaPayments = payments.filter(p => p.payment_system === 'yukassa' || p.payment_system === 'yukassa_auto' || !p.payment_system).length;
    const cryptocloudPayments = payments.filter(p => p.payment_system === 'cryptocloud').length;

    // Статистика по автоплатежам
    const autoPayments = payments.filter(p => p.payment_system === 'yukassa_auto').length;
    const manualPayments = payments.filter(p => p.payment_system === 'yukassa' || !p.payment_system).length;
