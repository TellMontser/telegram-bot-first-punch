...
export async function addPaymentMethod(userId, paymentMethodId, type = 'card', cardMask = null) {
  try {
    console.log(`💳 Добавление способа оплаты для пользователя ${userId}: ${paymentMethodId}`);
    ...
    if (error) throw error;
    console.log(`✅ Способ оплаты добавлен для пользователя: ${userId}`);
    return data;
  } catch (error) {
...
export async function enableAutoPayments(userId, paymentMethodId) {
  try {
    console.log(`🔄 Включение автоплатежей для пользователя ${userId} с методом ${paymentMethodId}`);
    
    // Отключаем автоплатежи для всех других способов оплаты пользователя
    await supabase
      .from('payment_methods')
      .update({ 
        auto_payments_enabled: false,
    ...
    // Включаем автоплатежи для указанного способа оплаты
    const { data, error } = await supabase
      .from('payment_methods')
      .update({ 
    ...
    if (error) throw error;
    console.log(`✅ Автоплатежи включены для пользователя: ${userId}`);
    return data;
  } catch (error) {
...
export async function disableAutoPayments(userId) {
  try {
    console.log(`🛑 Отключение автоплатежей для пользователя ${userId}`);
    ...
    if (error) throw error;
    console.log(`✅ Автоплатежи отключены для пользователя: ${userId}`);
  } catch (error) {
...
export async function getActiveAutoPaymentMethod(userId) {
  try {
    const { data, error } = await supabase
      .from('payment_methods')
      .select('*')
    ...
    console.error('❌ Ошибка при получении активного способа автоплатежа:', error);
    return null;
  }
}
