...
export async function addSubscription(userId, paymentId, amount, duration = 30) {
  try {
    console.log(`💳 Создание подписки для пользователя: ${userId}`);
...
export async function addPayment(userId, paymentId, amount, status = 'pending', confirmationUrl = null) {
  try {
    console.log(`💰 Добавление платежа: ${paymentId} для пользователя ${userId}`);
