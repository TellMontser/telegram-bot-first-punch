...
export async function updateUserPaymentData(userId, paymentData) {
  try {
    console.log(`💳 Обновление платежных данных пользователя: ${userId}`);
    ...
    if (error) throw error;
    console.log(`✅ Платежные данные обновлены для пользователя: ${userId}`);
    return true;
  } catch (error) {
    console.error('❌ Ошибка при обновлении платежных данных:', error);
    throw error;
  }
}

export async function getUserPaymentData(userId) {
  try {
    const { data, error } = await supabase
      .from('users')
      .select('email, phone, full_name, first_name, last_name')
      .eq('id', userId)
      .single();
    ...
    console.error('❌ Ошибка при получении платежных данных пользователя:', error);
    return null;
  }
}
...
// ==================== ФУНКЦИИ ДЛЯ РАБОТЫ С СПОСОБАМИ ОПЛАТЫ ====================

export async function addPaymentMethod(userId, paymentMethodId, type = 'card', cardMask = null, autoPaymentsEnabled = false) {
  try {
    console.log(`💳 Добавление способа оплаты для пользователя: ${userId}`);
    
    // Проверяем, есть ли уже такой способ оплаты
    ...
    if (existingMethod) {
      console.log(`⚠️ Способ оплаты ${paymentMethodId} уже существует`);
    ...
    const { data, error } = await supabase
      ...
      .insert({
        ...
        card_mask: cardMask,
        is_active: true,
        auto_payments_enabled: autoPaymentsEnabled,
        auto_payment_interval: 300, // 5 минут в секундах
        next_payment_date: autoPaymentsEnabled ? new Date(Date.now() + 300 * 1000).toISOString() : null,
        created_at: new Date().toISOString(),
        updated_at: new Date().toISOString()
      })
      .select()
    ...
    if (error) throw error;
    console.log(`✅ Способ оплаты добавлен: ${paymentMethodId}`);
    return data;
  } catch (error) {
...
export async function getPaymentMethod(paymentMethodId) {
  try {
    ...
    console.error('❌ Ошибка при получении способа оплаты:', error);
    return null;
  }
}
...
export async function getUserPaymentMethods(userId) {
  try {
    ...
    console.error('❌ Ошибка при получении способов оплаты пользователя:', error);
    return [];
  }
}

export async function enableAutoPayments(paymentMethodId) {
  try {
    console.log(`🔄 Включение автоплатежей для способа оплаты: ${paymentMethodId}`);
    
    const { error } = await supabase
      .from('payment_methods')
      .update({ 
        auto_payments_enabled: true,
        next_payment_date: new Date(Date.now() + 300 * 1000).toISOString(), // Следующий платеж через 5 минут
        updated_at: new Date().toISOString()
      })
      .eq('payment_method_id', paymentMethodId);

    if (error) throw error;
    console.log(`✅ Автоплатежи включены для способа оплаты: ${paymentMethodId}`);
    return true;
  } catch (error) {
    console.error('❌ Ошибка при включении автоплатежей:', error);
    return false;
  }
}

export async function disableAutoPayments(paymentMethodId) {
  try {
    console.log(`🚫 Отключение автоплатежей для способа оплаты: ${paymentMethodId}`);
    ...
    if (error) throw error;
    console.log(`✅ Автоплатежи отключены для способа оплаты: ${paymentMethodId}`);
    return true;
  } catch (error) {
    console.error('❌ Ошибка при отключении автоплатежей:', error);
    return false;
  }
}

export async function updateNextPaymentDate(paymentMethodId, nextDate) {
  try {
    const { error } = await supabase
      .from('payment_methods')
      .update({ 
    ...
    console.error('❌ Ошибка при обновлении даты следующего платежа:', error);
    return false;
  }
}

export async function getActiveAutoPaymentMethods() {
  try {
    ...
    console.error('❌ Ошибка при получении активных автоплатежей:', error);
    return [];
  }
}
...
export async function addSubscription(userId, paymentId, amount, duration = 30, paymentSystem = 'yukassa', paymentMethodId = null) {
  try {
    console.log(`💳 Создание подписки для пользователя: ${userId} через ${paymentSystem}`);
    
    // Получаем текущую подписку пользователя для подсчета месяцев подряд
    const currentSubscription = await getUserSubscription(userId);
    let consecutiveMonths = 1;
    let subscriptionType = 'manual';
    
    if (currentSubscription && currentSubscription.status === 'active') {
      // Если есть активная подписка, увеличиваем счетчик
      consecutiveMonths = (currentSubscription.consecutive_months || 0) + 1;
    } else {
      // Проверяем, была ли предыдущая подписка недавно (в пределах 35 дней)
      ...
      if (recentSubscriptions && recentSubscriptions.length > 0) {
        const lastSubscription = recentSubscriptions[0];
        const daysSinceLastSubscription = Math.floor(
          (Date.now() - new Date(lastSubscription.end_date).getTime()) / (1000 * 60 * 60 * 24)
        );
        
        if (daysSinceLastSubscription <= 35) {
          // Продолжаем счетчик
          consecutiveMonths = (lastSubscription.consecutive_months || 0) + 1;
        }
      }
    }
    
    // Если есть paymentMethodId, это автоплатеж
    if (paymentMethodId) {
    ...
    console.log(`✅ Подписка создана для пользователя: ${userId} через ${paymentSystem}, месяцев подряд: ${consecutiveMonths}`);
    return data;
  } catch (error) {
...
export async function getStats() {
  try {
    ...
    const autoSubscriptions = subscriptions.filter(sub => sub.subscription_type === 'auto').length;
    const manualSubscriptions = subscriptions.filter(sub => sub.subscription_type === 'manual' || !sub.subscription_type).length;
