...
// ==================== ФУНКЦИИ ДЛЯ РАБОТЫ С ПЛАТЕЖНЫМИ МЕТОДАМИ ====================

export async function savePaymentMethod(userId, paymentMethodId, type = 'card', cardMask = null) {
  try {
    console.log(`💾 Сохранение платежного метода для пользователя ${userId}: ${paymentMethodId}`);
    ...
    if (error) throw error;
    console.log(`✅ Платежный метод сохранен: ${paymentMethodId}`);
    return data;
  } catch (error) {
    console.error('❌ Ошибка при сохранении платежного метода:', error);
    throw error;
  }
}
...
export async function updatePaymentMethodStatus(userId, isActive) {
  try {
    console.log(`🔄 Обновление статуса платежных методов для пользователя ${userId}: ${isActive}`);
    
    const { error } = await supabase
      .from('payment_methods')
      .update({ 
        auto_payments_enabled: isActive,
        updated_at: new Date().toISOString()
      })
      .eq('user_id', userId);

    if (error) throw error;
    console.log(`✅ Статус платежных методов обновлен для пользователя: ${userId}`);
    return true;
  } catch (error) {
    console.error('❌ Ошибка при обновлении статуса платежных методов:', error);
    return false;
  }
}
...
export async function deletePaymentMethod(userId, paymentMethodId) {
  try {
    console.log(`🗑️ Удаление платежного метода ${paymentMethodId} для пользователя ${userId}`);
    ...
    if (error) throw error;
    console.log(`✅ Платежный метод удален: ${paymentMethodId}`);
    return true;
  } catch (error) {
    console.error('❌ Ошибка при удалении платежного метода:', error);
    return false;
  }
}
