  ...
  try {
    ...
    const { data, error } = await supabase
      ...
      .insert({
        ...
        subscription_type: 'recurring', // Теперь все подписки рекуррентные
...
export async function addPaymentMethod(userId, paymentMethodId, type = 'card', cardMask = null, autoPaymentsEnabled = true) {
  try {
    console.log(`💳 Добавление способа оплаты: ${paymentMethodId} для пользователя ${userId}`);
    
    // Сначала деактивируем все старые способы оплаты пользователя
    await supabase
    ...
    const { data, error } = await supabase
      ...
      .insert({
        ...
        auto_payment_interval: 600, // 10 минут в секундах
        next_payment_date: new Date(Date.now() + 10 * 60 * 1000).toISOString(), // Следующий платеж через 10 минут
        created_at: new Date().toISOString(),
        updated_at: new Date().toISOString()
      })
      .select()
    ...
    if (error) throw error;
    console.log(`✅ Способ оплаты добавлен: ${paymentMethodId} для пользователя ${userId}`);
    return data;
  } catch (error) {
...
export async function getPaymentMethodByUserId(userId) {
  try {
    const { data, error } = await supabase
      .from('payment_methods')
      .select('*')
...
export async function getActivePaymentMethods() {
  try {
    ...
    console.error('❌ Ошибка при получении активных способов оплаты:', error);
    return [];
  }
}

export async function updatePaymentMethodNextPayment(paymentMethodId, nextPaymentDate) {
  try {
    console.log(`🔄 Обновление даты следующего платежа: ${paymentMethodId} -> ${nextPaymentDate}`);
...
export async function disablePaymentMethod(userId, paymentMethodId) {
  try {
    console.log(`🚫 Отключение способа оплаты: ${paymentMethodId} для пользователя ${userId}`);
...
export async function getStats() {
  try {
    ...
    const [usersResult, messagesResult, joinRequestsResult, subscriptionsResult, paymentsResult, paymentMethodsResult] = await Promise.all([
    ...
    // Статистика по рекуррентным платежам
    const activeRecurringSubscriptions = paymentMethods.filter(pm => pm.auto_payments_enabled && pm.is_active).length;
    const totalRecurringRevenue = payments
      .filter(p => (p.status === 'succeeded' || p.status === 'paid') && p.payment_system === 'yukassa')
      .reduce((sum, p) => sum + Number(p.amount), 0);
    ...
    const stats = {
      ...
      yukassaPayments: payments.filter(p => p.payment_system === 'yukassa' || !p.payment_system).length,
      cryptocloudPayments: 0 // Убираем CryptoCloud
    };
