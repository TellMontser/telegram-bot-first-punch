...
export class PaymentScheduler {
  ...
  async processUserAutoPayment(user) {
    try {
      console.log(`💳 Обработка автоплатежа для пользователя ${user.telegram_id}, сумма: 1000 руб`);
...
⚠️ Доступ к закрытому каналу будет отозван при истечении текущего периода.
      ...
      await this.database.logSubscriptionAction(
        ...
        `Создан автоплатеж на 1000 руб. Payment ID: ${payment.id}`
      );
      ...
      console.log(`✅ Автоплатеж 1000 руб создан для пользователя ${user.telegram_id}: ${payment.id}`);
    } catch (error) {
