...
export class YookassaService {
  ...
  async createPayment(amount, description, returnUrl = null, savePaymentMethod = false) {
    ...
    // Добавляем confirmation только если нужен redirect
    if (returnUrl) {
      ...
      // Используем стандартную страницу ЮКассы
      paymentData.confirmation = {
        type: 'redirect'
        // Без return_url будет использована стандартная страница ЮКассы
      };
    }
  ...
  // Метод для проверки статуса ЮКассы
  async checkApiStatus() {
    try {
      console.log('🔍 Проверка статуса API ЮКассы...');
      
      // Попробуем получить информацию о магазине
