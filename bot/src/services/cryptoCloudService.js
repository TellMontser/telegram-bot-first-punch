...
export class CryptoCloudService {
  constructor() {
    this.apiKey = process.env.CRYPTOCLOUD_API_KEY;
    this.shopId = process.env.CRYPTOCLOUD_SHOP_ID;
    this.secret = process.env.CRYPTOCLOUD_SECRET;
    
    if (!this.apiKey || !this.shopId || !this.secret) {
      throw new Error('CryptoCloud настройки не найдены в переменных среды');
    }
  ...
  async createInvoice(amount, description, orderId = null, currency = 'RUB') {
    ...
    const invoiceData = {
      ...
      notification_url: `${process.env.WEBHOOK_URL}/webhook/callback`
    };
