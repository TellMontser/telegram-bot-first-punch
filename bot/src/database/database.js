...
export class Database {
  ...
  async createUser(telegramId, username, firstName, lastName, referralSource = null, referralLinkId = null) {
    try {
      ...
      let subscriptionAmount = 10; // Базовая сумма 10 рублей
      if (referralLinkId) {
        try {
          ...
          if (!error && referralLink) {
            subscriptionAmount = 10; // Всегда 10 рублей независимо от реферальной ссылки
      ...
      // Безопасное создание пользователя без проблемного поля
      const { data, error } = await this.supabase
        ...
        .insert({
          ...
          auto_payment_amount: 10 // Всегда 10 рублей
          // НЕ устанавливаем auto_payment_interval - пусть будет NULL или значение по умолчанию из БД
        })
        .select()
        .single();
      ...
      // После создания пользователя, безопасно обновляем auto_payment_interval
      try {
        const { error: updateError } = await this.supabase
          .from('users')
          .update({ auto_payment_interval: '2_minutes' })
          .eq('telegram_id', telegramId);
        
        if (updateError) {
          console.warn(`⚠️ Не удалось установить auto_payment_interval для пользователя ${telegramId}:`, updateError);
          // Не бросаем ошибку, пользователь уже создан
        } else {
          console.log(`✅ auto_payment_interval установлен для пользователя ${telegramId}`);
        }
      } catch (intervalError) {
        console.warn(`⚠️ Ошибка при установке auto_payment_interval:`, intervalError);
      }
  ...
  // Форматирование времени для Москвы
  ...
  async updateUserStatus(telegramId, status, subscriptionEnd = null) {
    // Если передано время окончания, конвертируем в московское время
    let moscowSubscriptionEnd = subscriptionEnd;
    if (subscriptionEnd) {
      const endDate = new Date(subscriptionEnd);
      moscowSubscriptionEnd = this.formatMoscowTime(endDate);
    }
    ...
    const { error } = await this.supabase
      ...
      .update({
        status: status,
        subscription_end: moscowSubscriptionEnd
      })
      .eq('telegram_id', telegramId);
  ...
  async updateAutoPaymentSettings(telegramId, amount, interval, customMinutes = null, nextPaymentDate = null) {
    // Принудительно устанавливаем сумму 10 рублей
    const fixedAmount = 10;
    
    // Проверяем допустимые значения интервала
    const allowedIntervals = ['2_minutes', '3_minutes', 'hourly', 'daily', 'weekly', 'monthly', 'custom'];
    if (!allowedIntervals.includes(interval)) {
      console.warn(`⚠️ Недопустимый интервал: ${interval}, используем 'monthly'`);
      interval = 'monthly'; // Безопасное значение по умолчанию
    }
    
    // Конвертируем дату в московское время если указана
    let moscowNextPaymentDate = nextPaymentDate;
    if (nextPaymentDate) {
      const nextDate = new Date(nextPaymentDate);
      moscowNextPaymentDate = this.formatMoscowTime(nextDate);
    }
    
    const updateData = { 
      auto_payment_amount: fixedAmount, // Всегда 10 рублей
    ...
    if (moscowNextPaymentDate) {
      updateData.next_payment_date = moscowNextPaymentDate;
    } else {
    ...
    try {
      ...
      console.log(`✅ Настройки автоплатежа обновлены для пользователя ${telegramId}:`, updateData);
    } catch (error) {
      console.error('❌ Критическая ошибка при обновлении настроек автоплатежа:', error);
      throw error;
    }
  }
  ...
  async updateUserStatusWithCustomEnd(telegramId, status, subscriptionEnd = null, customDuration = null) {
    ...
    if (customDuration && status === 'active') {
      ...
      if (durationMinutes) {
        finalSubscriptionEnd = this.formatMoscowTime(new Date(now.getTime() + durationMinutes * 60 * 1000));
      }
    }

    // Конвертируем в московское время если передано
    if (finalSubscriptionEnd && !customDuration) {
      const endDate = new Date(finalSubscriptionEnd);
      finalSubscriptionEnd = this.formatMoscowTime(endDate);
    }
  ...
  async updateLastPaymentDate(telegramId, date = null) {
    const moscowDate = date ? this.formatMoscowTime(new Date(date)) : this.formatMoscowTime(this.getMoscowTime());
    
    const { error } = await this.supabase
      .from('users')
      .update({ last_payment_date: moscowDate })
      .eq('telegram_id', telegramId);
  ...
  async getUsersReadyForAutoPayment() {
    ...
    // Убеждаемся, что у всех пользователей есть email для чека
    const usersWithEmail = (data || []).map(user => ({
      ...user,
      email: user.email || 'noreply@firstpunch.ru'
    }));
  ...
  // Расчет времени окончания подписки в московском времени (дублируем для совместимости)
