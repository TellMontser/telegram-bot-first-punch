...
export class Database {
  ...
  async createUser(telegramId, username, firstName, lastName, referralSource = null, referralLinkId = null) {
    try {
      ...
      const subscriptionAmount = await this.getSubscriptionAmount();
  ...
  // === МЕТОДЫ ДЛЯ УПРАВЛЕНИЯ НАСТРОЙКАМИ ПОДПИСКИ ===

  // Получение текущей суммы подписки
  async getSubscriptionAmount() {
    try {
      const { data, error } = await this.supabase
        .rpc('get_subscription_amount');

      if (error) {
        console.warn('⚠️ Ошибка получения суммы подписки, используем значение по умолчанию:', error);
        return 1000.00;
      }

      return data || 1000.00;
    } catch (error) {
      console.warn('⚠️ Ошибка получения суммы подписки:', error);
      return 1000.00;
    }
  }
  ...
  async getSubscriptionSettings() {
    try {
      ...
      console.error('❌ Ошибка получения настроек подписки:', error);
      return {
        id: 'default',
        subscription_amount: 1000.00,
        currency: 'RUB',
