      ...
      try {
        ...
        switch (data) {
          ...
          case 'channel_access':
            ...
            break;
          default:
            // Обрабатываем платежные callback'и
            if (data.startsWith('pay_yookassa_')) {
              const amount = parseFloat(data.replace('pay_yookassa_', ''));
              await this.handleYookassaPayment(chatId, amount);
            } else if (data.startsWith('pay_crypto_')) {
              const amount = parseFloat(data.replace('pay_crypto_', ''));
              await this.handleCryptoPayment(chatId, amount);
            } else {
...
📊 Проверить статус: /status
      ...
      const keyboard = [
        [{ text: '💳 Банковская карта (ЮКасса)', callback_data: `pay_yookassa_${subscriptionAmount}` }]
      ];
      
      // Добавляем криптоплатежи если сервис доступен
      if (this.cryptoCloudService) {
        keyboard.push([{ text: '₿ Криптовалюта (CryptoCloud)', callback_data: `pay_crypto_${subscriptionAmount}` }]);
      }

      await this.bot.sendMessage(chatId, 
        `💳 Выберите способ оплаты подписки:

💰 Сумма: ${subscriptionAmount} ₽${user.referral_source ? ` (специальная цена от ${user.referral_source})` : ''}
⏰ После первого платежа каждые 3 минуты будет списываться ${subscriptionAmount} ₽ автоматически
...
Выберите удобный для вас способ оплаты:`,
      ...
      console.log(`✅ Предложены способы оплаты для пользователя ${chatId}`);
    } catch (error) {
  ...
  async handleYookassaPayment(chatId, amount) {
    try {
      console.log(`💳 Создание ЮКасса платежа для пользователя ${chatId}, сумма: ${amount}`);
...
Нажмите кнопку ниже для перехода к оплате:`,
        {
          reply_markup: {
            inline_keyboard: [
              [{ text: '💳 Оплатить картой', url: payment.confirmation.confirmation_url }],
              [{ text: '🔙 Назад к выбору способа', callback_data: 'subscribe' }]
            ]
          }
        }
      );
      
      console.log(`✅ ЮКасса платеж создан для пользователя ${chatId}: ${payment.id}`);
    } catch (error) {
      console.error('❌ Ошибка при создании ЮКасса платежа:', error);
      await this.bot.sendMessage(chatId, '❌ Произошла ошибка при создании платежа через ЮКассу. Попробуйте позже.');
    }
  }

  // Обработка оплаты через CryptoCloud
  async handleCryptoPayment(chatId, amount) {
    try {
      console.log(`₿ Создание криптоплатежа для пользователя ${chatId}, сумма: ${amount}`);
      ...
      await this.database.createPayment(
        ...
        amount,
        'pending',
        null // payment_method_id для криптоплатежей пока null
      );
...
Нажмите кнопку ниже для перехода к оплате:`,
      ...
      console.log(`✅ CryptoCloud инвойс создан для пользователя ${chatId}: ${invoice.id}`);
    } catch (error) {
  ...
  async handlePaymentSuccess(paymentId) {
    try {
      ...
      try {
        ...
        // Если это не ЮКасса платеж, игнорируем ошибку
