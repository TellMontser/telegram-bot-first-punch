...
После активации подписки вы сможете повторно запросить доступ к каналу.
  ...
  async clearPreviousMessages(chatId) {
    try {
      // Пытаемся удалить последние несколько сообщений бота
      for (let i = 1; i <= 5; i++) {
        try {
          await this.bot.deleteMessage(chatId, Date.now() - i);
        } catch (error) {
...
*Выбери способ оплаты:*`;
    ...
    await this.bot.sendMessage(chatId, payMessage, {
      ...
      reply_markup: {
        inline_keyboard: [
          [{ text: '🏦 Оплатить через СБП', callback_data: 'pay_card_rf' }],
  ...
  async handleEmailInput(chatId, email) {
    try {
      ...
      if (!emailRegex.test(email)) {
        await this.bot.sendMessage(chatId, '❌ *Неверный формат email.* Попробуйте еще раз:', { parse_mode: 'Markdown' });
        return;
      }
