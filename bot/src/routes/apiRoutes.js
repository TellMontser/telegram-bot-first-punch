...
export function apiRoutes(database, telegramBot) {
  ...
  router.put('/users/:id/auto-payment-settings', async (req, res) => {
    try {
      ...
      console.log(`💰 API: Обновление настроек автоплатежа пользователя ${telegramId}: ${amount} руб, интервал: ${interval}`);
      ...
      const validIntervals = ['3_minutes', 'hourly', 'daily', 'weekly', 'monthly'];
      if (!validIntervals.includes(interval)) {
        return res.status(400).json({ error: 'Некорректный интервал' });
      }
      
      await database.updateAutoPaymentSettings(telegramId, amount, interval);
      ...
      if (user) {
        await database.logSubscriptionAction(
          ...
          `Настройки автоплатежа изменены администратором: ${amount} руб, интервал: ${interval}`
        );
      }
      ...
      if (telegramBot && telegramBot.bot) {
        try {
          ...
          const message = `💰 Настройки автоплатежа обновлены администратором:
