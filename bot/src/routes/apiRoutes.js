...
⏰ Новый период: ${intervalNames[interval]}`;
  ...
  // Удаление реферальной информации у пользователя
  router.post('/users/:id/remove-referral', async (req, res) => {
    try {
      const telegramId = parseInt(req.params.id);
      
      console.log(`🗑️ API: Удаление реферальной информации у пользователя ${telegramId}`);
      ...
      const oldReferralSource = user.referral_source;
      
      // Очищаем реферальную информацию
      ...
      await database.logSubscriptionAction(
        user.id,
        'admin_referral_removed',
        `Администратор удалил реферальную информацию: ${oldReferralSource}`
      );

      // Отправляем уведомление пользователю
      if (telegramBot && telegramBot.bot) {
        try {
          await telegramBot.bot.sendMessage(telegramId, `
ℹ️ Реферальная информация обновлена

Администратор удалил вашу реферальную информацию.

📊 Проверить профиль: /profile
          `);
          console.log(`📤 API: Уведомление об удалении реферала отправлено пользователю ${telegramId}`);
        } catch (botError) {
          console.warn(`⚠️ API: Не удалось отправить уведомление пользователю ${telegramId}:`, botError);
        }
      }
      
      console.log(`✅ API: Реферальная информация удалена у пользователя ${telegramId}`);
      res.json({ success: true, message: 'Реферальная информация удалена' });
    } catch (error) {
      console.error('❌ API: Ошибка при удалении реферальной информации:', error);
      res.status(500).json({ error: 'Ошибка сервера', details: error.message });
    }
  });
