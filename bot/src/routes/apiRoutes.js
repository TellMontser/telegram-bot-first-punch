...
export function apiRoutes(database, telegramBot) {
  ...
  router.get('/channel/requests', async (req, res) => {
    try {
      ...
      console.log('✅ API: Запросы канала получены:', mockRequests.length);
  ...
  router.get('/channel/stats', async (req, res) => {
    try {
      ...
      console.log('✅ API: Статистика канала получена:', mockStats);
      res.json(mockStats);
    } catch (error) {
  ...
  router.post('/channel/requests/:id/approve', async (req, res) => {
    try {
      ...
      // Здесь будет логика одобрения через Telegram Bot API
      if (telegramBot && telegramBot.bot) {
        // Получаем информацию о запросе из базы данных
        // const request = await database.getChannelRequest(requestId);
        // await telegramBot.bot.approveChatJoinRequest(telegramBot.PRIVATE_CHANNEL_ID, request.user_id);
      }
  ...
  router.post('/channel/requests/:id/decline', async (req, res) => {
    try {
      ...
      // Здесь будет логика отклонения через Telegram Bot API
      if (telegramBot && telegramBot.bot) {
        // const request = await database.getChannelRequest(requestId);
        // await telegramBot.bot.declineChatJoinRequest(telegramBot.PRIVATE_CHANNEL_ID, request.user_id);
      }
  ...
  router.post('/channel/requests/bulk-approve', async (req, res) => {
    try {
      ...
      // Здесь будет логика массового одобрения
      for (const requestId of requestIds) {
        // Логика одобрения каждого запроса
      }
      
      console.log(`✅ API: ${requestIds.length} запросов одобрено`);
      res.json({ success: true, message: `${requestIds.length} запросов одобрено` });
    } catch (error) {
  ...
  router.post('/channel/requests/bulk-decline', async (req, res) => {
    try {
      ...
      // Здесь будет логика массового отклонения
      for (const requestId of requestIds) {
        // Логика отклонения каждого запроса
      }
      
      console.log(`✅ API: ${requestIds.length} запросов отклонено`);
      res.json({ success: true, message: `${requestIds.length} запросов отклонено` });
    } catch (error) {
  ...
  router.post('/channel/kick-unregistered', async (req, res) => {
    try {
      ...
      console.log('✅ API: Незарегистрированные пользователи удалены');
      res.json({ success: true, message: 'Незарегистрированные пользователи удалены' });
    } catch (error) {
  ...
  router.get('/channel/members', async (req, res) => {
    try {
      ...
      // Здесь будет логика получения участников через Telegram Bot API
      const mockMembers = [];
      
      console.log('✅ API: Участники канала получены:', mockMembers.length);
