...
// Функция для создания автоплатежа с сохранением способа оплаты
export async function createAutoPayment(amount, description, returnUrl, metadata = {}) {
  try {
    console.log('🔄 Создание автоплатежа в ЮKassa:', { amount, description, returnUrl, metadata });
    ...
    const paymentData = {
      ...
      metadata: metadata,
      // Включаем сохранение способа оплаты для автоплатежей
      save_payment_method: true,
      // Добавляем чек для автоплатежа
      receipt: {
        customer: {
          full_name: metadata.first_name ? `${metadata.first_name} ${metadata.last_name || ''}`.trim() : 'Пользователь',
...
// Функция для создания повторного платежа с сохраненным способом оплаты
export async function createRecurringPayment(paymentMethodId, amount, description, metadata = {}) {
  try {
    console.log('🔄 Создание повторного платежа:', { paymentMethodId, amount, description, metadata });
    ...
    const paymentData = {
      ...
      description: description,
      metadata: metadata,
      // Добавляем чек для повторного платежа
      receipt: {
        customer: {
          full_name: metadata.first_name ? `${metadata.first_name} ${metadata.last_name || ''}`.trim() : 'Пользователь',
          email: metadata.email || 'user@firstpunch.ru'
    ...
    console.log('🔐 Отправка повторного платежа в ЮKassa API...');
    ...
    const responseText = await response.text();
    console.log(`📡 Ответ повторного платежа (${response.status}):`, responseText);
    ...
    if (!response.ok) {
      ...
      try {
        const errorJson = JSON.parse(responseText);
        errorDetails = errorJson.description || errorJson.error_description || errorJson.message || responseText;
        console.error('❌ Детали ошибки повторного платежа:', errorJson);
      } catch (parseError) {
        console.log('⚠️ Не удалось распарсить JSON ошибки');
      }
      
      console.error('❌ Ошибка повторного платежа:', response.status, errorDetails);
      throw new Error(`ЮKassa recurring payment error (${response.status}): ${errorDetails}`);
    }
    ...
    try {
      ...
      console.error('❌ Ошибка парсинга ответа повторного платежа:', parseError);
      throw new Error('Некорректный ответ от ЮKassa API');
    }
...
// Функция для создания простого платежа без чека (оставляем для совместимости)
